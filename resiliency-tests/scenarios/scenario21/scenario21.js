...
module.exports = function(runner, options) {
	...
	var scriptsArgs = [
		...
		'su0nvm0',
		'su1nvm0',
		'su2nvm0'//Name of the clusters to restart.
	];
		...
		run: function() {
			async.series([
				...
				async.apply(jmeter.run, 'jmeter/plans/insert_query.jmx', {
					...
					stage2duration: 4800, //80 min run (inserts + queries),
				...
				async.apply(
				  runner.runScripts,
				  'scenarios/scenario21/scenario21.sh',
				  `/home/${options.cluster.username}/scenario21.sh`,
				  scriptsArgs
				)
			],
			function (err, result) {
